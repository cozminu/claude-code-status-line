...
@test "pace_bar: hollow tick when behind pace, at the elapsed position" {
  [ "$(pace_bar 30 50)" = "███░░${REVERSE}▯${UNREVERSE}░░░░" ]
}

@test "pace_bar: solid tick when at or ahead of pace" {
  [ "$(pace_bar 60 50)" = "█████${REVERSE}▮${UNREVERSE}░░░░" ]
  [ "$(pace_bar 50 50)" = "█████${REVERSE}▮${UNREVERSE}░░░░" ]
}

@test "pace_bar: tick clamps inside the bar at both ends" {
  [ "$(pace_bar 100 100)" = "█████████${REVERSE}▮${UNREVERSE}" ]
  [ "$(pace_bar 10 0)" = "${REVERSE}▮${UNREVERSE}░░░░░░░░░" ]
}

@test "pace_bar: reverse-video wraps the tick even when buried deep in the filled run" {
  [ "$(pace_bar 90 50)" = "█████${REVERSE}▮${UNREVERSE}███░" ]
}
...
@test "usage_segment: dim label + pace-colored bar, green below pace" {
  ...
  [ "$(usage_segment 5h 30 1750009000 18000)" = "${DIM}5h${RESET} ${GREEN}███░░${REVERSE}▯${UNREVERSE}░░░░${RESET}" ]
}

@test "usage_segment: yellow when on pace, within ±tolerance" {
  STATUSLINE_NOW=1750000000
  # 50% at 50% elapsed: exactly on pace -> yellow
  [ "$(usage_segment 5h 50 1750009000 18000)" = "${DIM}5h${RESET} ${YELLOW}█████${REVERSE}▮${UNREVERSE}░░░░${RESET}" ]
}

@test "usage_segment: red when over pace (not orange, unlike 7d)" {
  STATUSLINE_NOW=1750000000
  # 60% at 50% elapsed: 10 points over tolerance -> red
  [ "$(usage_segment 5h 60 1750009000 18000)" = "${DIM}5h${RESET} ${RED}█████${REVERSE}▮${UNREVERSE}░░░░${RESET}" ]
}
...
@test "seven_day_segment: compact glyph on pace, expanded orange bar over pace" {
  ...
  [ "$(seven_day_segment 7d 60 1750302400 604800)" = "${DIM}7d${RESET} ${ORANGE}█████${REVERSE}▮${UNREVERSE}░░░░${RESET}" ]
}

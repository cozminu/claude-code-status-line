...
**Line 3** (optional, on by default via `STATUSLINE_SHOW_EMAIL`): the logged-in Claude account's email. This is the one segment that ignores the stdin payload entirely — `STDIN_PAYLOAD.md` documents no account/user field — and instead reads `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.claude.json` → `.oauthAccount.emailAddress` via `account_email()`, which fails closed to `""` (missing dir/file/field, malformed JSON) exactly like the other guarded segments. It renders unconditionally on `STATUSLINE_SHOW_EMAIL=1` whenever an email is present, regardless of which profile (`CLAUDE_CONFIG_DIR`) is active.
